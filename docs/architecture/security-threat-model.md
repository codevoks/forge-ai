...
| Budget reservation orphaned by worker crash | Fixed during final audit; protected and verified | A worker killed between `BudgetService.reserve()` and `settle()`/`release()` left the reservation permanently `reserved` and its usage permanently counted, while the retried attempt reserved again for the same logical work — a genuine resource-accounting drift (not an authorization bypass) found during the integrated final audit. `WorkerRepository.run_recovery_scan` now releases any `reserved` `budget_reservations` tied to a task whose stale attempt it reclaims, in the same transaction, and reverses the counted usage; verified by a dedicated test simulating the crash and asserting reconciliation |
| Duplicate queue delivery double-charging budget | Protected and verified | A tool invocation already `succeeded` returns via `ToolRuntime`'s idempotent reuse path before any reservation is attempted; a test redelivering every outbox message twice confirms exactly one settled reservation per distinct tool call, not two |
| Budget duplicate settlement | Protected and verified | `BudgetReservationRepository.settle` requires `status = 'reserved'`; settling an already-settled reservation raises `ProblemError(409)` before any usage adjustment, so usage is never decremented/incremented twice for one reservation |
| Agent decision schema injection (confused deputy via model output) | Protected and verified | `AgentDecision` (and its base `StrictModel`) is `pydantic` `extra="forbid"`; a model response carrying an unexpected field (e.g. attempting to smuggle a grant/approval/budget override) is rejected by schema validation before it reaches application logic |
